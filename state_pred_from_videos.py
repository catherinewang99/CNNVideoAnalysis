...
def main(configs=None):
    ...
    for f1, f2 in f1_f2_list:
        ...
        state_1_good_trials = pert_pred_utils.load_stim_i_good_trials_for_state_pred("state_1", configs['inst_trial_type'], configs['prep_root_dir'], f1, f2)
        state_2_good_trials = pert_pred_utils.load_stim_i_good_trials_for_state_pred("state_2", configs['inst_trial_type'], configs['prep_root_dir'], f1, f2)
        total_i_good_trials = np.array([state_1_good_trials, state_2_good_trials])

        if configs["states"] > 2:
            state_3_i_good_trials = pert_pred_utils.load_stim_i_good_trials_for_state_pred("state_3", configs['inst_trial_type'], configs['prep_root_dir'], f1, f2)

            total_i_good_trials = np.array([state_1_good_trials, state_2_good_trials, state_3_i_good_trials])
    ...
    # CW: Commented out because not needed AFAIK
    #for f1, f2 in f1_f2_list:
    #    # f1 = 'BAYLORGC[#]'
    #    # f2 = '[yyyy]_[mm]_[dd]'
    ...
    for f1, f2 in f1_f2_list:
        ...
        if (f1,f2) in few_error_trial_f1_f2_list: #CW: not relevant ATM
            continue
