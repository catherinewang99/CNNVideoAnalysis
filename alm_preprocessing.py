...
def main(args):
    ...
    data_prefix = 'Trial_Ordered_Long_Prep_2p'  # Pert_Normal_Bin0.1_Prep_, Pert_Normal_Prep_, Pert_Normal_Long_Prep_.
    ...
    elif data_prefix == 'Trial_Ordered_Long_Prep_2p':
        prep_kwargs = {'align_to': None, 'begin_time': -4.6, 'end_time': 3.3, 'bin_width': 0.4, 'stride': 0.1}
    ...
    if os.path.isfile(os.path.join(prep_save_path, 'trial_type_labels.npy')):
        ...
        #sess = Session(args.filename, parser=parsers.ParserNuoLiDualALM)
        sess = Session(args.filename, parser= parsers.ParserNuoLiALM)
        ...
        np.save(os.path.join(prep_save_path, 'cull_trial_mask.npy'), cull_trial_mask)
        np.save(os.path.join(prep_save_path, 'cull_unit_mask.npy'), cull_unit_mask)
