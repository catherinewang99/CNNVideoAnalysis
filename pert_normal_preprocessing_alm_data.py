...
for filename in filelist:
    ...
    mouse_label = split_filename[0][6:]

    '''
    BAYLORGC78kilosort_2019_07_01.mat: no right and bi stim trials, and also no pert_normal neurons at all!
    '''
    if filename in ['BAYLORGC17_2018_06_11.mat', 'BAYLORGC61_2019_02_22.mat', 'BAYLORGC61_2019_02_23.mat', \
    'BAYLORGC61_2019_02_24.mat', 'BAYLORGC85_2019_08_03.mat', 'BAYLORGC78kilosort_2019_07_01.mat',
    'BAYLORGC4kilosort_2018_03_23.mat', 'BAYLORGC50kilosort_2018_12_07.mat', 'BAYLORGC81kilosort_2019_09_16.mat', 'BAYLORGC83kilosort_2019_08_16.mat']: # These four are sessions sorted in both ways.
        continue
...
# filenames = filedict['GC4'] + filedict['GC12'] + filedict['GC13'] + filedict['GC15'] + filedict['GC18'] + filedict['GC17'] + filedict['GC19'] + \
# filedict['GC21'] + filedict['GC22'] + filedict['GC24'] + filedict['GC25'] + filedict['GC26'] + filedict['GC31'] + filedict['GC50'] + \
# filedict['GC61'] + filedict['GC66'] + filedict['GC69'] + filedict['GC71'] + \
# filedict['GC79'] + filedict['GC80'] + filedict['GC81'] + filedict['GC83'] + filedict['GC84'] + filedict['GC85'] + filedict['GC88']
...
def preprocess_bk(session, trial_mask=None, unit_mask=None, align_to=None, begin_time=-3.05, end_time=1.35, bin_width=0.1, stride=0.1, both_labels=False, label_by_report=True):
    '''
    ...
    if unit_mask is None:
        unit_mask = np.array([True]*session.neural_data.shape[0])

    if trial_mask is None:
        trial_mask = np.array([True]*session.neural_data.shape[1])
...
def train_test_split(rates, labels, trial_type_labels, train_prop, shuffle=True):
    '''
...
def save_by_stim_type(rates, labels, trial_type_labels, mask, prep_save_path, stim_type, train_prop, shuffle=True):
    ...
    train_rates, train_labels, train_trial_type_labels, test_rates, test_labels, test_trial_type_labels = train_test_split(stim_rates, stim_labels, stim_trial_type_labels, train_prop, shuffle)
...
def find_pert_normal_neuron_mask(no_stim_rates, left_stim_rates, right_stim_rates, bi_stim_rates, stim_bin_min, stim_bin_max, neural_unit_location):
...
def pert_normal_screening(bin_centers, rates, sess, bin_width, pyramidal_only):
    ...
    if not np.isclose(((bin_centers-bin_width/2)-np.nanmin(sess.stim_on_time))[stim_bin_min], 0, atol=1e-2) and ((bin_centers-bin_width/2)-np.nanmin(sess.stim_on_time))[stim_bin_min] < 0:
        print('warning: np.isclose(((bin_centers-bin_width/2)-np.nanmin(sess.stim_on_time))[stim_bin_min], 0, atol=1e-2) is False.')
        stim_bin_min = stim_bin_min+1 

    stim_bin_max = np.abs((bin_centers+bin_width/2)-np.nanmin(sess.stim_off_time)).argmin()
    if not np.isclose(((bin_centers+bin_width/2)-np.nanmin(sess.stim_off_time))[stim_bin_max], 0, atol=1e-2) and ((bin_centers+bin_width/2)-np.nanmin(sess.stim_off_time))[stim_bin_max] > 0:
        print('warning: np.isclose(((bin_centers+bin_width/2)-np.nanmin(sess.stim_off_time))[stim_bin_max], 0, atol=1e-2) is False.')
        stim_bin_max = stim_bin_max-1    

    pert_normal_neuron_mask = find_pert_normal_neuron_mask(no_stim_rates, left_stim_rates, right_stim_rates, bi_stim_rates, stim_bin_min, stim_bin_max, neural_unit_location)
...
def main(args):
    ...
    data_prefix = 'All_Pert_Normal_Trial_Ordered_Long_Prep_' # Pert_Normal_Bin0.1_Prep_, Pert_Normal_Prep_, Pert_Normal_Long_Prep_.
    ...
    if os.path.isfile(os.path.join(prep_save_path, 'trial_type_labels.npy')):
        ...
        cull_trial_mask, cull_unit_mask = sess.cull_data()

        # bin_centers, rates, labels = preprocess_bk(sess, label_by_report=True)
        ...
        if 'Pert_Normal' in data_prefix:
            rates, neural_unit_location, neural_unit_type = pert_normal_screening(bin_centers, rates, sess, bin_width=prep_kwargs['bin_width'], pyramidal_only=pyramidal_only)
        else:
            neural_unit_location = sess.neural_unit_location.copy()
            neural_unit_type = sess.neural_unit_type.copy()
    ...
    if 'Trial_Ordered' in data_prefix:
        train_rates, train_labels, train_trial_type_labels, test_rates, test_labels, test_trial_type_labels = train_test_split(rates, labels, trial_type_labels, args.train_prop, shuffle=False)
    else:
        train_rates, train_labels, train_trial_type_labels, test_rates, test_labels, test_trial_type_labels = train_test_split(rates, labels, trial_type_labels, args.train_prop, shuffle=True)
    ...
    if 'Trial_Ordered' in data_prefix:
        save_by_stim_type(rates, labels, trial_type_labels, no_stim_mask, prep_save_path, 'no_stim', args.train_prop, shuffle=False)
        save_by_stim_type(rates, labels, trial_type_labels, left_stim_mask, prep_save_path, 'left_stim', args.train_prop, shuffle=False)
        save_by_stim_type(rates, labels, trial_type_labels, right_stim_mask, prep_save_path, 'right_stim', args.train_prop, shuffle=False)
        save_by_stim_type(rates, labels, trial_type_labels, bi_stim_mask, prep_save_path, 'bi_stim', args.train_prop, shuffle=False)

    else:
        save_by_stim_type(rates, labels, trial_type_labels, no_stim_mask, prep_save_path, 'no_stim', args.train_prop, shuffle=True)
        save_by_stim_type(rates, labels, trial_type_labels, left_stim_mask, prep_save_path, 'left_stim', args.train_prop, shuffle=True)
        save_by_stim_type(rates, labels, trial_type_labels, right_stim_mask, prep_save_path, 'right_stim', args.train_prop, shuffle=True)
        save_by_stim_type(rates, labels, trial_type_labels, bi_stim_mask, prep_save_path, 'bi_stim', args.train_prop, shuffle=True)
...
if __name__ == '__main__':
    if args.filename:
        ...
        print('n_sess: {}'.format(len(new_filenames)))
        for sess_iter, filename in enumerate(new_filenames):
            args.filename = filename
            print('')
            print('Session {}'.format(sess_iter+1))
            print('filename: {}'.format(filename))
