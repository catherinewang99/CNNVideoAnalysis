...
def main():
    ...
    params = {'batch_size': configs['bs'], 'shuffle': True, 'num_workers': configs['num_workers'], 'pin_memory': bool(configs['pin_memory'])} if use_cuda else {}
    ...
    elif sess_type == 'full_delay':
        f1_list = list(os.listdir(video_root_path))
    else:
    ...
    for f1 in os.listdir(target_path):
        ...
        for f2 in os.listdir(os.path.join(target_path, f1)):
            ...
            with open(os.path.join(target_path, f1, f2, 'all_neurons_{}.obj'.format(configs['fr_percentile'])), 'rb') as read_file:
    ...
    for f1 in os.listdir(target_path):
        ...
        for f2 in os.listdir(os.path.join(target_path, f1)):
            ...
            with open(os.path.join(target_path, f1, f2, 'all_neurons_{}.obj'.format(int(configs['fr_percentile']))), 'rb') as read_file:
                all_neurons = pickle.load(read_file)
    ...
    for f1 in f1_list:
        ...
        for f2 in os.listdir(os.path.join(video_root_path, f1)):
            ...
            for i in n_trial_types_list:
                ...
                cur_target = target_dict[f1][f2][i] # an array of shape (n_trials, T, max_n_neurons). Some entries filled with np.nan if n_neurons < max_n_neurons.
    ...
    if '_input_channel' in configs.keys() and configs['_input_channel'] != 1:
        ...
        n_pre_frames = (n_input_frames-1)//2 # 4
        n_post_frames = (n_input_frames-1)//2 # 4

        if (n_input_frames-1)%2 != 0:
            n_pre_frames += 1

        # out of range for early times.
        if (configs['pred_timesteps'][0] - n_pre_frames) < 0: # 0 - 4 < 0
    ...
    for cv_ind in range(n_cv):
        ...
        train_video_path[cv_ind], train_target[cv_ind], train_sess_inds[cv_ind], train_trial_type[cv_ind],\
        test_video_path[cv_ind], test_target[cv_ind], test_sess_inds[cv_ind], test_trial_type[cv_ind],\
        sess_wise_test_video_path_dict[cv_ind], sess_wise_test_target_dict[cv_ind], sess_wise_test_sess_inds_dict[cv_ind], sess_wise_test_trial_type_dict[cv_ind],\
...
def train_test_helper_cv(f1_list, cv_ind, configs, video_root_path, n_trial_types_list,\
    ...
    sess_wise_test_video_path_dict, sess_wise_test_target_dict, sess_wise_test_sess_inds_dict, sess_wise_test_trial_type_dict,\
    selected_frames, transform_list, params, mouse_sort, video_path_dict, target_dict, sess_inds_dict, device):

    train_set = MyDatasetNeuReg(video_root_path, train_video_path, train_target, train_sess_inds, train_trial_type, selected_frames, \
        configs['view_type'], transform_list=transform_list, img_type=configs['img_type'])
    valid_set = MyDatasetNeuReg(video_root_path, test_video_path, test_target, test_sess_inds, test_trial_type, selected_frames, \
        configs['view_type'], transform_list=transform_list, img_type=configs['img_type'])
        ...
        for f2 in os.listdir(os.path.join(video_root_path, f1)):
            ...
            for i in n_trial_types_list:
                ...
                cur_valid_set = MyDatasetNeuReg(video_root_path, cur_test_video_path, cur_test_target, \
                    cur_test_sess_inds, cur_test_trial_type, selected_frames, configs['view_type'], \
    ...
    model_save_path = os.path.join(configs['models_cv_dir'], configs['sess_type'], 'fr_percentile_{}'.format(int(configs['fr_percentile'])),\
     img_type_str, neural_time_str, 'view_type_{}'.format(configs['view_type'][0]), 'model_name_{}'.format(configs['model_name']), \
        'cnn_channel_list_{}'.format(cnn_channel_str), 'fc_layer_{}'.format(fc_layer_str), 'cv_ind_{}'.format(cv_ind))

    logs_path = os.path.join(configs['logs_cv_dir'], configs['sess_type'], 'fr_percentile_{}'.format(int(configs['fr_percentile'])), \
        img_type_str, neural_time_str, 'view_type_{}'.format(configs['view_type'][0]), 'model_name_{}'.format(configs['model_name']),\
        'cnn_channel_list_{}'.format(cnn_channel_str), 'fc_layer_{}'.format(fc_layer_str), 'cv_ind_{}'.format(cv_ind))
    ...
    for epoch in range(configs['epochs']):
        ...
        epoch_test_loss, epoch_test_score = validation(configs, model, device, valid_loader, best_test_score, loss_fct, model_save_path)
    ...
    for f1 in f1_list:
        ...
        for f2 in os.listdir(os.path.join(video_root_path, f1)):
            ...
            for i in n_trial_types_list:
                ...
                print('f2: ', f2)
                print('i: ', i)
                _, sess_trial_type_wise_best_test_scores[f1][f2][i] = validation(configs, model, device, cur_valid_loader, \
                    float('inf'), loss_fct)
    ...
    print('Total average trial_type_cond R2 (for sessions and trial types that has at least {} test samples): {:.2f}'.format(configs['n_test_samples_thr'], \
